...
from keras.layers import Conv1D, Conv2D, Conv3D, concatenate
...
def non_local_block(ip, intermediate_dim=None, compression=2,
    ...
    """
    channel_dim = 1 if KB.image_data_format() == 'channels_first' else -1
    ip_shape = KB.int_shape(ip)
...
def non_local_block_concate(ip1,ip2, intermediate_dim=None, compression=2,
                    mode='embedded', add_residual=True):
    """
    Adds a Non-Local block for self attention to the input tensor.
    Input tensor can be or rank 3 (temporal), 4 (spatial) or 5 (spatio-temporal).
    ...
    # g path
    g1 = _convND(ip, rank, intermediate_dim)
    g1 = Reshape((-1, intermediate_dim))(g1)
    g2 = _convND(ip, rank, intermediate_dim)
    g2 = Reshape((-1, intermediate_dim))(g2)
    if compression > 1 and mode == 'embedded':
        # shielded computation
        g1 = MaxPool1D(compression)(g1)
        g2 = MaxPool1D(compression)(g2)

    # compute output path
    y1 = dot([f, g1], axes=[2, 1])
    y2 = dot([f, g2], axes=[2, 1])

    # reshape to input tensor format
    if rank == 3:
        y1 = Reshape((dim1, intermediate_dim))(y1)
        y2 = Reshape((dim1, intermediate_dim))(y2)
    elif rank == 4:
        if channel_dim == -1:
            y1 = Reshape((dim1, dim2, intermediate_dim))(y1)
            y2 = Reshape((dim1, dim2, intermediate_dim))(y2)
        else:
            y1 = Reshape((intermediate_dim, dim1, dim2))(y1)
            y2 = Reshape((intermediate_dim, dim1, dim2))(y2)
    else:
        if channel_dim == -1:
            y1 = Reshape((dim1, dim2, dim3, intermediate_dim))(y1)
            y2 = Reshape((dim1, dim2, dim3, intermediate_dim))(y2)
        else:
            y1 = Reshape((intermediate_dim, dim1, dim2, dim3))(y1)
            y2 = Reshape((intermediate_dim, dim1, dim2, dim3))(y2)

    # project filters
    y1 = _convND(y1, rank, channels)
    y2 = _convND(y2, rank, channels)
